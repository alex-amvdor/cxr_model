...
print(tabulate(
    ...
    headers=["setup", "value"], tablefmt="github"))
print()
...
for thickness, label in ((290.0, "29 nm film"), (1e7, "1 mm bulk")):
    ...
    rows.append(["A idealized Feranchuk", label, N_line_A, 0.0,
                 N_line_A, spec_A.max(), note])
    ...
    rows.append(["B Monte Carlo (Zhai)", label, line_cts, brem_cts,
                 line_cts + brem_cts, peak, ""])

    ratios.append([label, N_line_A / line_cts,
                   "should be ~1 (idealized model valid here)"
                   if thickness < 1e4 else
                   "idealized bulk extrapolation overshoots"])

print(tabulate(
    rows,
    headers=["model", "target", "line\n[cts/s/nA]", "brem\n[cts/s/nA]",
             "total\n[cts/s/nA]", "peak\n[Phs/eV/s/nA]", "note"],
    tablefmt="github", floatfmt=(None, None, ".1f", ".1f", ".1f", ".3f")))
print()
print(tabulate(ratios, headers=["target", "A/B line counts", "interpretation"],
               tablefmt="github", floatfmt=(None, ".2f")))
print()
print(f"Count rates are unity-QE detector estimates over {BAND[0]:.0f}-"
