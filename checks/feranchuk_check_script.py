...
for el in ("Li", "F"):
    if el not in Z_TABLE:
        raise RuntimeError(
            f"Add '{el}' to atomic_form_factors (Z, Cromer-Mann, "
...
print(f"LiF L_abs at {E_line:.0f} eV: {L_abs:.1f} Angstrom (film L_z = {L_z_ang:.0f} Angstrom)")
...
N_model = flux_per_second(
    ...
    polarization="both",
    use_henke=True,
    geometry="lif",  # <-- the two that matter
)
